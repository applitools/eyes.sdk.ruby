...
# require 'applitools/universal_client/socket'
# require 'applitools/universal_client/eyes_manager'
  ...
  module UniversalClient
    class UniversalClient
      ...
      def initialize(queue = EM::Queue.new)
        @socket = Applitools::UniversalClient::Socket.new
        @queue = queue
        prepare_socket
      ...
      def make_manager(eyes_manager_config)
        ...
        Applitools::UniversalClient::EyesManager.new(eyes_manager, self)
      end
      ...
      def prepare_socket
        socket_uri = ::Applitools::UniversalClient::UniversalServer.check_or_run
      ...
      def connect_and_configure_socket(uri)
        Thread.new do
          EM.run do
            @socket.connect(uri)
            @socket.emit(SESSION_INIT, { name: 'rb', version: ::Applitools::VERSION, protocol: 'webdriver' })
          end
        end
      end
